...
def main():
    ...
    while True:
        try:
            ...
            prices_close = deque(priceData['Close'].tail(5), maxlen=5)
            prices_high = deque(priceData['High'].tail(5), maxlen=5)
            prices_low = deque(priceData['Low'].tail(5), maxlen=5)
            prices_open = deque(priceData['Open'].tail(5), maxlen=5)
...
def get_signal(prices_close, prices_open):
    ...
    pips_per_candle = calculate_pips_per_candle(prices_open, prices_close, pip_size=0.0001)

    for pips, signal in enumerate(pips_per_candle,start=1):
        print(f"{pips}: {signal}")


def calculate_pips_per_candle(prices_open, prices_close, pip_size=0.0001):
    ...
    for open_price, close_price in zip(prices_open, prices_close):
        ...
        for i in range(len(pips_per_candle) - 2):
            if abs(pips_per_candle[i]) >= 3 and abs(pips_per_candle[i+1]) >= 3 and abs(pips_per_candle[i+2]) >= 3:
                longSignal = True
                buy_long()
                break
